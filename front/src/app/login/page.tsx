...
export default function Login() {
    ...
    async function verificaLogin(data: Inputs) {
        ...
        if (response.status === 200) {
            ...
            localStorage.setItem("tipo", dados.tipo); // 👈 importante!

            // Redireciona para a rota correta com base no tipo
            if (dados.tipo === "admin") {
                router.push("/admin");
            } else {
                router.push("/");
            ...
            <section className="w-full rounded-lg sm:max-w-md p-8">
                <div className="space-y-6">
                    ...
                    <form className="space-y-4" onSubmit={handleSubmit(verificaLogin)}>
                        <div>
                            <label htmlFor="email" style={{ color: "rgba(20, 200, 113, 1)" }} className="block mb-1 text-sm">
                                Digite seu email
                            </label>
                            <input
                                type="email"
                        ...
                        <div>
                            <label htmlFor="password" style={{ color: "rgba(20, 200, 113, 1)" }} className="block mb-1 text-sm">
                                Senha
                            </label>
                            <input
                                type="password"
                        ...
                        <div className="flex justify-between text-sm">
                            ...
                            <a href="#" style={{ color: "rgba(20, 200, 113, 1)" }} className="hover:underline">
                                Esqueceu sua senha?
                            </a>
                        </div>
                        <button
                        ...
                        <p className="text-sm text-center text-gray-400">
                            Novo na GoodTicket?{" "}
                            <Link href="/cadastro" style={{ color: "rgba(20, 200, 113, 1)" }} className="hover:underline">
                                Cadastrar
                            </Link>
                        </p>
                    </form>
